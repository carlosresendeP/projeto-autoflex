...
import { addMaterial, editMaterial } from "../store/materialSlice";
import {
  materialSchema,
...
export const MaterialForm = ({ onClose, initialData }: MaterialFormProps) => {
  ...
  } = useForm<MaterialFormInput, MaterialFormData>({
  ...
  const handleSave = async (data: MaterialFormData) => {
    try {
      if (isEditing) {
        await dispatch(editMaterial({ ...initialData, ...data })).unwrap();
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <form
          onSubmit={handleSubmit(
            handleSave as unknown as SubmitHandler<MaterialFormInput>,
          )}
        ...
        >
          <div>
            ...
            <input
              {...register("code")}
              className="w-full mt-1 p-3 border rounded-lg border-slate-200 outline-none focus:ring-2 focus:ring-emerald-100"
            />
            {errors.code && (
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              ...
              className="flex-1 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 shadow-lg shadow-emerald-200 disabled:opacity-50"
            >
