...
export const CompositionForm = ({ onClose }: { onClose: () => void }) => {
  ...
  } = useForm<CompositionFormInput, CompositionFormData>({
  ...
  const handleSave = async (data: CompositionFormData) => {
    try {
      ...
      console.error(error);
      toast.error(
        "Erro ao salvar composição. Verifique se essa combinação já existe.",
      );
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <form
          onSubmit={handleSubmit(
            handleSave as unknown as SubmitHandler<CompositionFormInput>,
          )}
